...
float get_height(ivec2 heightmap_pos) {
    ...
    ivec3 sample_uv = ivec3(in_tile_pos.x >> lod_level, tile_id);
    return height = texelFetch(heightmap, sample_uv, 0).r;
}

void main() {
    ivec2 terrain_pos = get_terrain_pos();
