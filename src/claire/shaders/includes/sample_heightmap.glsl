...
bool get_height(sampler2DArray[LOD_COUNT] heightmap, isampler2D tile_id_lookup, ivec2 heightmap_pos, int lod_level, out float height) {
...
bool get_connected_height(sampler2DArray[LOD_COUNT] heightmap, isampler2D tile_id_lookup, ivec2 heightmap_pos, int lod_level, ivec2 mesh_pos, ivec4 neighbor_lod_nwse, out float height) {
    ivec2 sample_lod = heightmap_pos >> lod_level;

    bool activate_north = mesh_pos.y == 0;
    bool activate_south = mesh_pos.y == 1 << MESH_SIZE_EXPONENT;
    ...
    bool activate_east = mesh_pos.x == 1 << MESH_SIZE_EXPONENT;
    ...
    float height_1;
    bool valid_1 = get_height(heightmap, tile_id_lookup, sample_world_1, lod_level, height_1);
    float height_2;
    bool valid_2 = get_height(heightmap, tile_id_lookup, sample_world_2, lod_level, height_2);
...
bool get_height_with_normal(sampler2DArray[LOD_COUNT] heightmap, isampler2D tile_id_lookup, ivec2 terrain_pos, int lod_level, ivec2 mesh_pos, ivec4 neighbor_lod_nwse, out float height, out vec3 normal) {
    float height_center;
    bool valid_center = get_connected_height(heightmap, tile_id_lookup, terrain_pos, lod_level, mesh_pos, neighbor_lod_nwse, height_center);

    int weight_ns = 2;
    float height_north;
    bool valid_north = get_connected_height(heightmap, tile_id_lookup, terrain_pos + ivec2(0, -1), lod_level, mesh_pos, neighbor_lod_nwse, height_north);
    height_north = int(valid_north) * height_north + (1 - int(valid_north)) * height_center;
    weight_ns = weight_ns - int(!valid_north);
    float height_south;
    bool valid_south = get_connected_height(heightmap, tile_id_lookup, terrain_pos + ivec2(0, 1), lod_level, mesh_pos, neighbor_lod_nwse, height_south);
    ...
    int weight_we = 2;
    float height_west;
    bool valid_west = get_connected_height(heightmap, tile_id_lookup, terrain_pos + ivec2(-1, 0), lod_level, mesh_pos, neighbor_lod_nwse, height_west);
    height_west = int(valid_west) * height_west + (1 - int(valid_west)) * height_center;
    weight_we = weight_we - int(!valid_west);
    float height_east;
    bool valid_east = get_connected_height(heightmap, tile_id_lookup, terrain_pos + ivec2(1, 0), lod_level, mesh_pos, neighbor_lod_nwse, height_east);
