...
class _TestLodData(LodData):
    def __init__(self, aabb, max_error_y):
        self.aabb = aabb
...
def test_error_below_threshold_renders(selector, aabb):
    assert selector.should_refine(_TestLodData(aabb, 0.01)) == "render"


def test_error_above_threshold_refines(selector, aabb):
    assert selector.should_refine(_TestLodData(aabb, 100.0)) == "refine"


def test_error_at_threshold_has_defined_behavior(selector, aabb):
    result = selector.should_refine(_TestLodData(aabb, 10.0))

    assert result in {"render", "refine", "use_previous"}


def test_error_inside_hysteresis_returns_use_previous(selector, aabb):
