...
_GREYSCALE_IMAGE: Final[Path] = Path(__file__).parent / "overlay_picture_greyscale.jpg"
_COLOR_IMAGE: Final[Path] = Path(__file__).parent / "overlay_picture_color.jpg"
...
def _load_heightmap() -> HeightmapData:
    if not _PATH.exists():
...
def main() -> None:
    ...
    window.show()
    viewer.load_heightmap(_load_heightmap())
    viewer.set_overlay_image(_GREYSCALE_IMAGE)
